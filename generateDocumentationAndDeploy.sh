...
# conan install -o xmsgridtrace:pybind=True -s compiler.version=6 -s compiler.libcxx=libstdc++11 -if ./conan -g txt xmsgridtrace/${TRAVIS_TAG}@aquaveo/stable 
conan install -o xmsgridtrace:pybind=True -s compiler.version=6 -s compiler.libcxx=libstdc++11 -if ./conan -g txt xmsgridtrace/1.0.2@aquaveo/stable 
# get the path to the conan package
export PATH_TO_PYTHON_PACKAGE=$(cat ./conan/conanbuildinfo.txt | grep PYTHONPATH.*xmsgridtrace | sed -r 's/^PYTHONPATH=\["(.*?)"\]$/\1/')
...
if [ -d "html" ] && [ -f "html/index.html" ] && [ -f "html/pydocs/index.html" ]; then
    mv xmsgridtrace.tag "$TRAVIS_BUILD_DIR/code_docs/$GH_REPO_NAME/"
